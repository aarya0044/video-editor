...
export default function AuthPage({ mode = "login" }) {
  ...
  // If user is already logged in, go straight to editor
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) nav("/editor");
    });

    // Listen for OAuth redirect coming back
    const { data: listener } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === "SIGNED_IN" && session) nav("/editor");
    });
    ...
    <div className="auth-root">
      ...
      <div className="auth-card">
        ...
        {tab !== "forgot" && (
          <div className="auth-tabs">
            ...
            <button className={`auth-tab ${tab==="signup"?"active":""}`} onClick={() => { setTab("signup"); setErr(""); setMsg(""); }}>Create account</button>
          </div>
        )}
          ...
          {tab !== "forgot" && (
            <div className="field">
              <div className="field-row">
                ...
                {tab === "login" && (
                  <button type="button" className="forgot-link" onClick={() => { setTab("forgot"); setErr(""); setMsg(""); }}>
                    Forgot password?
                  </button>
                )}
              </div>
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        .auth-back { position:fixed; top:24px; left:24px; display:flex; align-items:center; gap:7px; color:var(--t2); text-decoration:none; font-size:13px; background:rgba(255,255,255,.04); border:1px solid var(--b1); padding:7px 14px; border-radius:8px; transition:all .2s; z-index:10; }
        ...
        .auth-tab { flex:1; padding:8px; border:none; background:transparent; color:var(--t2); font-family:'DM Sans',sans-serif; font-size:13px; cursor:pointer; border-radius:7px; transition:all .2s; }
        ...
        .pass-eye { position:absolute; right:12px; top:50%; transform:translateY(-50%); background:none; border:none; color:var(--t3); cursor:pointer; padding:4px; display:flex; align-items:center; }
